...
Vector4ds doublesToVector4ds(const doubles& values)
{
    ...
    for (size_t i = 0; i < values.size(); i += 4)
        clippingPlanes.push_back({values[i], values[i + 1], values[i + 2], values[i + 3]});
...
Vector3d sphereFilling(const double radius, const uint64_t occurrence, const uint64_t occurrences, const uint64_t rnd,
                       Vector3d& position, Quaterniond& rotation, const double ratio)
{
...
{
    const double radius = innerRadius + (rand() % 1000 / 1000.0) * (1.0 - innerRadius);
    double phi = M_PI * ((rand() % 2000 - 1000) / 1000.0);
    double theta = M_PI * ((rand() % 2000 - 1000) / 1000.0);
    Vector3ds points;
    for (size_t i = 0; i < nbPoints; ++i)
    {
        Vector3d point = {radius * sin(phi) * cos(theta), radius * sin(phi) * sin(theta), radius * cos(phi)};
...
{
    return Vector3d(v.x - m * floor(v.x / m), v.y - m * floor(v.y / m), v.z - m * floor(v.z / m));
}
...
{
    return cos(index * M_PI / 180.0) + sin(index * M_PI / 45.0) + cos(index * M_PI / 72.0);
}

Quaterniond weightedRandomRotation(const Quaterniond& q, const uint64_t seed, const uint64_t index, const double weight)
{
    const Quaterniond qPitch = angleAxis(weight * rnd2(seed + index * 2), Vector3d(1.0, 0.0, 0.0));
    const Quaterniond qYaw = angleAxis(weight * rnd2(seed + index * 3), Vector3d(0.0, 1.0, 0.0));
    const Quaterniond qRoll = angleAxis(weight * rnd2(seed + index * 5), Vector3d(0.0, 0.0, 1.0));
