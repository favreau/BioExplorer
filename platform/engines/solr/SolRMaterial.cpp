...
void SolRMaterial::commit()
{
    ...
    auto kernel = SolRContext::get().getKernel();
    kernel->setMaterial(_id, _diffuseColor.x, _diffuseColor.y, _diffuseColor.z, _glossiness, _reflectionIndex,
                        _refractionIndex, procedural, wireframe, wireframeDepth, transparency, _opacity,
                        diffuseTextureId, normalTextureId, bumpTextureId, specularTextureId, reflectionTextureId,
                        transparencyTextureId, ambientOcclusionTextureId, _specularColor.x, _specularColor.y,
                        _specularColor.z, _emission, _emission, _emission, false);
    resetModified();
}
