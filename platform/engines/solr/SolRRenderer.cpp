...
SolRRenderer::SolRRenderer(const AnimationParameters& animationParameters,
...
{
    auto solrFrameBuffer = std::static_pointer_cast<SolRFrameBuffer>(frameBuffer);
    auto lock = solrFrameBuffer->getScopeLock();

    auto kernel = SolRContext::get().getKernel();
    kernel->render_begin(0);
    kernel->render_end();
...
void SolRRenderer::commit()
{
    ...
    auto& postProcessingInfo = kernel->getPostProcessingInfo();
...
Renderer::PickResult SolRRenderer::pick(const Vector2f& pickPos)
{
    ...
    auto kernel = SolRContext::get().getKernel();
    const auto id = kernel->getPrimitiveAt(pickPos.x, pickPos.y);
    if (id != 0)
    {
        result.hit = true;
        result.pos.x = id;
