...
// Permutation table. This is a randomly shuffled array of all the integers from 0 to 255 inclusive.
static __device__ const int permutation[] = {
...
static __device__ const int perm[512] = {
...
// Gradient function
static __device__ float grad(int hash, float x, float y, float z)
{
    int h = hash & 15;                                  // Take the first 4 bits of the hash code
    float u = h < 8 ? x : y;                            // If the first bit is 0, use x. Otherwise, use y.
    float v = h < 4 ? y : (h == 12 || h == 14 ? x : z); // If the first two bits are 0, use y. If the second bit is 1,
                                                        // and the third bit is 0, use x. Otherwise, use z.
    return ((h & 1) == 0 ? u : -u) +
           ((h & 2) == 0 ? v : -v); // Use the last 2 bits to determine the signs of the vectors.
}

static __device__ float fade(float t)
{
    return t * t * t * (t * (t * 6 - 15) + 10);
}

static __device__ float perlinNoise(const float3& p)
{
    float3 point = p;
    ...
    float result =
        ::optix::lerp(w,
                      ::optix::lerp(v,
                                    ::optix::lerp(u, grad(perm[AA], point.x, point.y, point.z),
                                                  grad(perm[BA], point.x - 1, point.y, point.z)),
                                    ::optix::lerp(u, grad(perm[AB], point.x, point.y - 1, point.z),
                                                  grad(perm[BB], point.x - 1, point.y - 1, point.z))),
                      ::optix::lerp(v,
                                    ::optix::lerp(u, grad(perm[AA + 1], point.x, point.y, point.z - 1),
                                                  grad(perm[BA + 1], point.x - 1, point.y, point.z - 1)),
                                    ::optix::lerp(u, grad(perm[AB + 1], point.x, point.y - 1, point.z - 1),
                                                  grad(perm[BB + 1], point.x - 1, point.y - 1, point.z - 1))));

    return (result + 1.0) / 2.0; // Normalize result to [0, 1]
}
